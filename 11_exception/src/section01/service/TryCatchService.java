...
public class TryCatchService {
	...
	public void test3() {
		try {
			...
			// ArithmeticException과 InputMismatchException 
			// 공통 부모 타입을 매개 변수로 선언

			// - try에서 예외 발생 시 "예외 객체 참조 주소"가 던져짐!
		
			// - 업 캐스팅 : 부모 타입 참조변수 = 자식 객체 참조 주소;
			// 								RuntimeException = "예외 객체 참조 주소"
	...
	public void test4() {
		try {
			...
			// ArrayIndexOutOfBoundsException : 배열 인덱스 위 초과
			
			/* 부모 타입 예외 처리 구문을 먼저 작성하면 
			 * 자식 타입 예외까지 모두 잡기 때문에
			 * 뒤에 있는 자식 catch문이 절대로 실행 될 수 없다!!!
			 * ex) 부모 타입이 작성되면 그아래 자식 타입은 작성 불가
			 *  작성할거면 자식타입 작성하고 부모타입(제일밑) 작성*/ 
//		} catch(RuntimeException e) {
...
//		}
		 ...
		 *  
		 *  - 예외 강제 발생 구문throw : throw new 예외 클래스();
		 *  
		 */
		public void test5() {
		...
		try {
			...
			input = sc.nextInt(); // 다른거입력시 InputMismatchException 발생 할 수있다 
			...
			System.out.println("예외 최상위 부모 Exception 으로");
