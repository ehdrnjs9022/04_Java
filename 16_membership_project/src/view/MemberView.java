...
public class MemberView {
	...
	public void addMember() throws IOException {
		...
		while(true) {
			...
			if(phone.length() == 11) {
				...
				if(result)	System.out.println("*** 회원이 추가 되었습니다 ***");
	...
	public void selectName() throws IOException {
		...
		List<Member> selectName = service.selectName(name);
		List<Member> memberList = service.getMemberList();
		
		if(memberList == selectName) {
			memberList.stream().forEach(member -> {
				System.out.println(member);
			});
		} else {
	...
	public void updateAmount() throws IOException {
		...
		if(memberList1.size() > 1) {
			...
			for(int i =0; i < memberList1.size() ; i++) {
				System.out.println((i+1) + ")" + memberList1.get(i).getName() + memberList1.get(i).getPhone()) ;
			
			}
				System.out.print("선택할 회원의 번호를 입력 : ");
				int num = sc.nextInt(); 
				
				t=memberList1.get(num-1);
		}
		
		System.out.print("누적할 금액 입력 : ");
		int acc = sc.nextInt();
		
		service.updateAmount(t, acc);
