...
//	- 같은 자료형 변수의 묶음
	...
	//ex) new int[4]; 
		// int형 변수(기본자료형->값 저장) 4개 묶음 배열 생성

	//ex) new StudentDTO[5];
	// StudentDTO 변수(참조형->주소 저장) 5개 묶음 배열 생성
	...
	private void allStudent() {
		...
		// StudentService 객체에서 모든 학생을 저장한 객체 배열 얻어와
		// 화면에 출력
		...
		for(StudentDTO std : students) {
			 ...
			 * 컴파일러가 자동으로 참조변수.toStiring()을 호출하도록 변경
			 */
			
			System.out.println(std);//sdt.toString()
	...
	private void addStudent() {
		...
		// StudentDTO 객체 생성하기
		StudentDTO student =  new StudentDTO(studentTnumber, name, gender);
	 ...
	 *  - 이름을 입력 받아
	 *  반환형       클래스명.메서드명(매개 변수)
	 *  Student DTO StudentService.selectName(String name) 호출
	 *  
	 *  - 학생 정보 반환 받아서 출력
	 *  단, 반환 받은 학생이 없을경우(null)
		 "000 학생이 존재하지 않습니다" 출력
	 *</pre>
	 * 
	 */
	 ...
	 *  HTML, CSS, JS, Java 점수를 순서대로 입력 받아 후
	...
	private void updateIndex() {
		...
		// sc.next() / sc.next000()
		// - 띄어쓰기 / 엔터 입력 전까지만 인식 ex) 10띄어쓰기(HTML) 20 30 40
		...
		// 반환형 void (없음) 앞 내용에 학생이 무조건 있을시 가정하는 함수적어서
		service.updateSocre(index, html, css, js, java);
	...
	private void deleteStudent() {
		...
		System.out.print("삭제 할 인덱스 번호 입력 :");
		int index = sc.nextInt();
