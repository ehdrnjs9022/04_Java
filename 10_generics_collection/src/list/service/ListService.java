...
public class ListService {
	...
	public void test2() {
		...
		//Elemnet
		// (E e) -> 메서드의 E(소괄호안에)는 "갈비탕"(e) 라는것이 "String"(E)타입이다
		// E(괄호밖)제너릭 반환값 -> String타입으로 적용되어 반환중
		
		/* void(반환형없다) add(int index, E e) : index 번째에 e 추가(중간 삽입)*/
		menuList.add(1,"갈비탕");
		
		/* String toString() : 리스트에 저장된 값을 하나의 문자열로 반환*/
		System.out.println(menuList.toString());
		
		// E(괄호밖)제너릭 반환값 -> 수정된 타입 E (현재는String)타입이다
		/* E set(int index, E e) 
		 * - index 번재 요소에 전달 받은 e를 세팅(수정)
	...
	// E(괄호밖)제너릭 반환값 -> 삭제된 타입 E (현재는String)타입이다
		/* E remove(int index)
		 * - index번째 요소 삭제
		 * - 삭제된 요소(E) 반환됨
	...
	public void test3() {
		...
		// 중간 추가/수정/삭제가 빠른 LinkedList 로 교체
		...
		//	try {
		//		Thread.sleep(1000); // 현재 스레드를 100ms 동안 멈춤
