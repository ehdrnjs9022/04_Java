...
public class FileService {
	...
	/** 
	 * File 클래스를 이용한 객체 생성 + 풀더 관리
	 */
	public void test1() {
		
		// 셀지로 파일폴더의 유무 관계 없이 일단 참조!
		...
		if(!directory.exists()) { // 폴더가 없다면
			...
			File parent = new File(directory.getParent()); // 부모 풀도 참조
	...
	/* 절대 경로 : 절대적 기준(root폴더)(/, C:, D:)  최상위폴더 부터 경로 표기
	 * 
	...
	public void test3() {
		...
		File[] files = folder.listFiles();
				for(File f: files) {
					...
					// 문자열 출력 // %-20s -(왼쪽정렬) kb -> 삼항연산자 null일때 빈칸 or kb?
					System.out.printf(
							"%-20s %-20s %-5s %-10s\n",
