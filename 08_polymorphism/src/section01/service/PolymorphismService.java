...
//		else if(obj instanceof Object) { 
	...
	public Object getInstance(int num) {
		...
		// if, for, while의
		// {} 내부 코드가 한 줄인 경우 {} 생략 가능 -> 메소드,클래스는 불가
		if(num == 1) return new Child(); // num = 1 이면 child객체를 만들어서 반환한다
		
		if(num == 2) return new Parent();
		
		return new Object(); // if 문 둘다 안걸리면 Objcet로 객체생성하겠다
	}
	...
	public void test2() {
		...
		// o1, o2는 업캐스팅 상태
		Object o1 = getInstance(1); // 실제로는 Child 객체 반환
	...
	public void test4() {
		...
		for(Object obj : arr ) { // 향상된 for문  배열중 꺼낼것 : 배열
		 ...
		 * - 코드를 수행 중 지정된 지점(break point)에서
		 *  실행을 멈추고 
		 *  해당 시점에 조재하는 변수/필드 값을 확인하는 모드
		 */
	}
