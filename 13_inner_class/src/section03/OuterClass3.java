...
public class OuterClass3 {
	...
	public void outerDisplay() {
		...
		// 외부 클래스(outerDisplay)의 멤버 메서드에 선언된 지역 변수
		...
		class LocalInerClass{
			...
			public void innerDisplay() {
				// 외부 클래스(outerDisplay)의 멤버 변수, 지역 변수 접근 가능!
				System.out.println(instanceMessage + localMessage);
			}
		}
		
		/* 지역 내부 클래스를 이용해 객체 생성 */
		LocalInerClass inner = new LocalInerClass();
		inner.innerDisplay();
	}
	// 다른 메서드에서는 지역 내부 클래스 호출 불가
	public void method() {
		//LocalInerClass inner = new LocalInerClass(); 오류발생
